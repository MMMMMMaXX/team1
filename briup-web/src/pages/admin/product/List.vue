<template>
  ...
  <div class="product">
    <div class="header">
      ...
      <el-input
        v-model="productName"
        v-searchDebounceAndThrotFromMx="queryProduct"
        style="width: 200px"
        placeholder="请输入产品名称"
        clearable
      />
      ...
      <!-- 查询按钮 -->
      <el-button v-buttonThrotFromMx="queryProduct" type="primary" round>查询</el-button>
    </div>
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" @close="dialogClose">
      ...
      <el-form ref="product_form" :model="productForm" :rules="rules" label-width="80px">
        <!-- 产品名称 -->
        ...
        <el-form-item label="所属栏目" prop="productCategoryId">
          <el-select v-model="productForm.productCategoryId" clearable placeholder="请选择栏目">
            <el-option
              v-for="category in categoryArr.filter((item) => !item.parentId)"
              :key="category.id"
              :label="category.name"
  ...
  methods: {
    submitForm() {
      this.$refs['product_form'].validate(async(valid) => {
        if (valid) {
          // 发生表单验证的请求 提交表单对象
          const res = await post('/product/saveOrUpdate', this.productForm)
          if (res.status === 200) {
            this.$message({
              message: res.message,
              type: 'success'
    ...
    async clickOffline(id) {
      const res = await get('/product/offline', { id })
      if (res.status === 200) {
        this.$message({
          message: res.message,
          type: 'success'
    ...
    async clickOnline(id) {
      const res = await get('/product/online', { id })
      if (res.status === 200) {
        this.$message({
          message: res.message,
          type: 'success'
    ...
    async getAllProductData() {
      ...
      // 通过对应的参数发送网络请求
      const res = await get('/product/pageQuery', { ...data, ...this.queryData })
